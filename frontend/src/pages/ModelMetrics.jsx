    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-6xl mx-auto py-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-2">Evaluation metrics for the ShelfLife AI Regressor</p>
        </div>
        <Link to="/" className="px-4 py-2 bg-surface text-white border border-white/10 hover:bg-white/5 rounded-xl transition-all font-medium">
          Back to Home
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-surface/50 p-6 rounded-xl border border-white/5 flex items-center gap-4 hover:border-blue-500/30 transition-colors">
          ...
          <div>
            ...
            <h3 className="text-2xl font-bold">{(metrics.r2_score * 100).toFixed(2)}%</h3>
          </div>
        </div>
        ...
        <div className="bg-surface/50 p-6 rounded-xl border border-white/5 flex items-center gap-4 hover:border-orange-500/30 transition-colors">
          ...
          <div>
            <p className="text-slate-400 text-sm">Root Mean Squared Error (RMSE)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-surface rounded-xl border border-white/5 p-6">
          <h3 className="text-xl font-semibold mb-6">Feature Importance (Top 10)</h3>
            ...
            <div className="h-[350px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={distributionData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="predicted" stroke="#10b981" fillOpacity={1} fill="url(#colorPv)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ) : (
